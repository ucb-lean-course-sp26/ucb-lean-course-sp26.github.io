...
lemma extend_proper (D : ℕ) (S : Finset V) (v : V) (hv : v ∈ S)
  ...
  · -- Case B: x ≠ v
    ...
    -- now do case analysis on where y = v or not (in class exercise)
    sorry
...
theorem colorable_of_degree_le (D : ℕ) (hdeg : ∀ v, G.degree v ≤ D) (S : Finset V) :
  ...
  | @insert a s ha ih =>
      ...
      have hErase : (insert a s).erase a = s := by
        ext x
        by_cases hx : x = a
        · simp [hx, ha]           -- x = a: not in erase, not in s
        · simp [Finset.mem_erase, hx]  -- x ≠ a: erase keeps x iff insert had x iff s has x

      -- To finish, we lift the coloring from s to (insert a s) by coloring a freshly
      -- which is guaranteed to be possible by extend_proper
...
lemma exists_shorter_walk_of_not_nodup {u v : V} (p : Walk (G := G) u v) :
  ...
  | cons h_adj p_rest ih =>
      ...
      have hcases : u ∈ p_rest.toList ∨ ¬ p_rest.toList.Nodup := by
        have hnd : ¬ (u ∉ p_rest.toList ∧ p_rest.toList.Nodup) := by
          simpa [Walk.toList, List.nodup_cons] using h_dup
        simpa using not_and_or.mp hnd
...
theorem exists_short_walk {u v : V} (p_orig : Walk (G := G) u v) :
  ...
  have hP : ∀ n, P n := by
    ...
    · have h_not_nodup : ¬ p.toList.Nodup := by
        intro h_nodup
        -- A nodup list of V-vertices has length ≤ |V|.
        have h1 : p.toList.length ≤ Fintype.card V :=
          nodup_length_le_card (V := V) h_nodup
        -- But p.toList.length = p.length + 1, so p.length < |V| — contradiction.
        have h2 : p.length + 1 ≤ Fintype.card V := by
          simpa [Walk.length_toList] using h1
        exact h_small (Nat.lt_of_lt_of_le (Nat.lt_succ_self p.length) h2)
